...
const blankURL = `http://resource/client/system/empty/html/index.html`;
let _currentEvents: { eventName; callback}[] = [];
let _cursorCount = 0;
...
export class View extends alt.WebView {
    private constructor(url, isOverlay = false) {
        super(url, isOverlay);
        this.isVisible = false;
    }

    static async getInstance(url, addCursor, isInit = false, blurBackground = false){
        if (!_instance) {
    ...
    public on(eventName, listener: (...args: any[]) => void) {
    ...
    public emit(eventName, ...args: any[]) {
        super.emit(eventName, ...args);
    }

    public showCursor(state) {
        if (state) {
            _cursorCount += 1;
            try {
                alt.showCursor(true);
