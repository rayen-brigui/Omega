...
var thirst_timer = setInterval(()=>{ //-83 thirst per tick.
    alt.Player.all.forEach(player => {
       // const playertempval = dbstore.CharacterTempValue(player.id, "thirsthungercontrol");
       
            const current_thirst = player.getMeta('waterlvl');
            if(current_thirst >= 20) {
                const newthirst = current_thirst - 20;
                const newthirstpercent = ((newthirst - 0) * 100) / (1000 - 0);
                
                player.setMeta('waterlvl',newthirst)
...
alt.on('playerDeath', (victim, killer, weaponHash) => {
    ...
    if (weaponHash === 0xcd274149) {
        ...
        alt.emitClient(victim, "hunger", 0);
        alt.emitClient(victim, "thirst", 0);
...
alt.on('ShowHud', (player) => {
    alt.emitClient(player, "hunger",player.getMeta('foodlvl'));
    alt.emitClient(player, "thirst", player.getMeta('waterlvl'));
});
